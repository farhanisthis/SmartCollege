...
import { categorizeContent, formatContent, analyzeImage, processContentWithFiles, } from "./services/ai";
import { aiManager } from "./services/aiManager";
import { processInput } from "./services/inputPipeline";
import { textExtractionService, TextExtractionService, } from "./services/textExtraction";
...
export async function registerRoutes(app) {
    ...
    app.post("/api/auth/login", async (req, res) => {
        try {
            ...
            if (user.password !== password) {
                ...
                // Debug log length to check for hidden characters
                console.log(`[Login Debug] Input len: ${password.length}, Stored len: ${user.password.length}`);
                return res
                    .status(401)
                    .json({ message: "Invalid username or password" });
            }
    ...
    app.put("/api/profile", requireAuth, async (req, res) => {
        try {
            const { name, phone, location, bio, department, year, rollNumber } = req.body;
    ...
    app.put("/api/preferences", requireAuth, async (req, res) => {
        try {
            ...
            const updatedUser = await storage.updatePreferences(req.session.userId, preferences);
            if (!updatedUser) {
                return res.status(404).json({ message: "User not found" });
            }
            res.json({
    ...
    app.post("/api/updates", requireCR, upload.array("files", 10), // Increased from 5 to 10 files
    handleUploadError, async (req, res) => {
        try {
            console.log("[updates] Received create request");
            const { content, originalContent, category, priority, isUrgent, dueDate, } = req.body;
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("AI pipeline timeout (30s)")), 30000));
            let processed;
            try {
            ...
            const { title: processedTitle, formattedContent: processedContent, category: detectedCategory, isUrgent: detectedIsUrgent, dueDate: detectedDueDate, deadlineDate: detectedDeadlineDate, tags, } = processed;
    ...
    app.post("/api/updates/unified", requireCR, upload.array("files", 10), handleUploadError, async (req, res) => {
        try {
            ...
            if (files && files.length > 0) {
                ...
                for (const file of files) {
                    try {
                        if (TextExtractionService.isSupportedFileType(file.originalname)) {
                            const extracted = await textExtractionService.extractText(file.path);
                            extractedTexts.push({
                                fileName: file.originalname,
                                content: extracted.content,
                                metadata: extracted.metadata,
                            });
                            console.log(`[unified-upload] Extracted text from ${file.originalname}`);
                        }
                        else {
                            ...
                            console.log(`[unified-upload] Unsupported file type: ${file.originalname}`);
                        }
                    }
                    catch (error) {
                        console.error(`[unified-upload] Error extracting text from ${file.originalname}:`, error);
            ...
            let processed;
            try {
                processed = await processContentWithFiles(contextText || "", extractedTexts);
            }
            catch (error) {
    ...
    app.post("/api/ai/test", requireCR, async (req, res) => {
        try {
            ...
            console.log(`[ai-test] Testing AI with provider: ${provider || "auto-fallback"}`);
    ...
    app.get("/api/test/gemini-v1", async (req, res) => {
        try {
            ...
            const result = await aiManager.useGemini("gemini-2.5-flash", "Say hello in one word");
            if (result.success) {
                console.log(`[gemini-v1-test] ✅ SUCCESS: Using v1 API with provider: ${result.provider}`);
                res.json({
                    success: true,
    ...
    app.get("/api/files/:filename", requireAuth, async (req, res) => {
        try {
            ...
            console.log("File request for:", filename, "from user:", req.session.userId);
            ...
            console.log("Serving file:", filename, "download mode:", download, "MIME type:", file?.mimeType);
            ...
            if (download) {
                ...
                res.setHeader("Content-Disposition", `inline; filename="${file?.originalName || filename}"`);
    ...
    app.get("/api/preview/:filename", requireAuth, async (req, res) => {
        try {
            const { filename } = req.params;
            const filePath = getFilePath(filename);
            console.log("Preview request for:", filename, "from user:", req.session.userId);
            if (!fs.existsSync(filePath)) {
                console.log("File not found:", filePath);
                return res.status(404).json({ message: "File not found" });
            }
            ...
            console.log("Serving preview file:", filename, "with MIME type:", file?.mimeType);
            // Send file
            res.sendFile(path.resolve(filePath));
        }
        catch (error) {
    ...
    app.post("/api/admin/regenerate-descriptions", requireAuth, async (req, res) => {
        try {
            ...
            for (const update of allUpdates) {
                try {
                    ...
                    if (!contentToAnalyze || contentToAnalyze.trim().length < 10) {
                        errors.push(`Update ${update.id}: Insufficient content to analyze`);
                        errorCount++;
                        continue;
                    }
                    ...
                    const result = await formatContent(contentToAnalyze, categoryResult);
                    if (result && result.content && result.content.trim().length > 0) {
                        ...
                        errors.push(`Update ${update.id}: AI failed to generate description`);
                        errorCount++;
                    }
                }
                catch (error) {
                    errors.push(`Update ${update.id}: ${error instanceof Error ? error.message : "Unknown error"}`);
                    errorCount++;
                }
            }
            res.json({
...
`;
            ...
            const result = await aiManager.generateWithFallback(enhancementPrompt, "gemini");
