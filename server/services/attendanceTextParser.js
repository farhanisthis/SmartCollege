...
export class AttendanceTextParser {
     ...
     */
    async parseAttendanceText(ocrText, contextHints) {
        try {
            console.log(`[Attendance Parser] Processing ${ocrText.length} characters of OCR text`);
            ...
            if (!aiResult.success) {
                console.error(`[Attendance Parser] AI processing failed:`, aiResult.error);
                console.log(`[Attendance Parser] Falling back to regex-based parsing...`);
            ...
            console.log(`[Attendance Parser] Successfully parsed ${parsedData.data.length} student records`);
            return {
                success: true,
     ...
     */
    parseWithRegexFallback(ocrText) {
        try {
            ...
            for (const line of lines) {
                ...
                if (match) {
                    ...
                    attendanceMarkers.forEach((marker, index) => {
                        subjects[`Subject_${index + 1}`] = marker;
                    });
                    students.push({
                        enrollNo,
                        name,
            ...
            if (students.length > 0) {
                console.log(`[Regex Parser] Successfully parsed ${students.length} students`);
                return {
                    success: true,
                    data: students,
            ...
            return {
                success: false,
                error: `Regex parsing failed: ${error instanceof Error ? error.message : String(error)}`,
            };
        }
    }
    /**
     ...
     */
    createParsingPrompt(ocrText, contextHints) {
...
Return ONLY the JSON array:`;
    ...
    extractSubjects(entry) {
        ...
        for (const [key, value] of Object.entries(entry)) {
            ...
            const isKnownSubject = knownSubjects.includes(key) ||
    ...
    calculateConfidence(data, rawText) {
        ...
        const validEnrollments = data.filter((entry) => entry.enrollNo !== "MISSING" && /^\d{11}$/.test(entry.enrollNo)).length;
        score += Math.min(validEnrollments * 5, 40);
        // Score for meaningful names
        const meaningfulNames = data.filter((entry) => entry.name !== "Unknown Student" && entry.name.length > 3).length;
        score += Math.min(meaningfulNames * 3, 20);
        // Score for subject data
        const totalSubjectEntries = data.reduce((sum, entry) => sum + Object.keys(entry.subjects).length, 0);
