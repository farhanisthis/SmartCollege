...
export function parseAttendanceSheetBasic(extractedText, fileName) {
    ...
    for (const line of lines) {
        ...
        // Skip header lines
        if (trimmedLine.toLowerCase().includes("institute") ||
        ...
        const enrollmentPattern = /^(\d{8,15})\s+([A-Za-z\s\.]+?)\s*([PA]|Present|Absent)?\s*([PA]|Present|Absent)?\s*$/i;
        ...
        if (enrollmentMatch) {
            ...
            const studentStatus = status1?.toUpperCase().startsWith("P") ||
        ...
        const nameStatusPattern = /^([A-Za-z\s\.]{3,})\s*([PA]|Present|Absent)\s*([PA]|Present|Absent)?\s*$/i;
