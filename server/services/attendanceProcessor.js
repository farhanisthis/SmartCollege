 ...
 */
function convertAttendanceFormat(data) {
    // Check if data has attendance_records (new format)
    if (data.attendance_records && Array.isArray(data.attendance_records)) {
        console.log(`[AttendanceProcessor] Converting from Gemini format with ${data.attendance_records.length} records`);
        const subjects = ["Cloud Computing", "Computer Graphics"]; // Default subjects for CC and CG
        const students = data.attendance_records.map((record) => {
            ...
            return {
                ...
                rollNumber: record.enroll_no && record.enroll_no !== "—"
                    ? record.enroll_no
                    : undefined,
                studentId: record.enroll_no && record.enroll_no !== "—"
                    ? record.enroll_no
                    : undefined,
 ...
 */
export async function processAttendanceSheet(extractedText, fileName, date) {
    try {
        ...
        console.log(`[AttendanceProcessor] Extracted text preview: ${extractedText.substring(0, 500)}...`);
...
${extractedText}`;
        ...
        if (!aiResult.success || !aiResult.data) {
            console.log(`[AttendanceProcessor] AI failed, using basic parser fallback`);
        ...
        console.log(`[AttendanceProcessor] AI Response: ${aiResponse.substring(0, 200)}...`);
        ...
        try {
            ...
            console.log(`[AttendanceProcessor] AI response parsing failed, using basic parser fallback`);
            ...
            return {
                ...
                data: basicResult,
                warnings: [
                    `Failed to parse AI response: ${parseError instanceof Error
                        ? parseError.message
                        : String(parseError)}`,
                    "Used basic parser as fallback",
                ],
 ...
 */
function normalizeAttendanceData(data, dateOverride) {
    const normalized = { ...data };
    const processingNotes = [...(data.processingNotes || [])];
    // Normalize date
    if (dateOverride) {
    ...
    if (normalized.subjects) {
        normalized.subjects = normalized.subjects.map((subject) => {
    ...
    if (normalized.students) {
        normalized.students = normalized.students.map((student) => {
            ...
            if (normalizedStudent.subjects) {
                ...
                Object.entries(normalizedStudent.subjects).forEach(([subject, status]) => {
                    const normalizedSubject = subjectMapping[subject.toUpperCase().replace(/\s+/g, "")] ||
                        subject;
 ...
 */
export async function matchStudentsToDatabase(students) {
    try {
        ...
        console.log(`[AttendanceProcessor] Found ${dbStudents.length} students in database`);
        const matchedStudents = students.map((student) => {
            ...
            const match = dbStudents.find((dbStudent) => {
                ...
                if (studentNameParts.length >= 2 && dbNameParts.length >= 2) {
                    const commonParts = studentNameParts.filter((part) => dbNameParts.some((dbPart) => dbPart.includes(part) || part.includes(dbPart)));
            ...
            if (match) {
                console.log(`[AttendanceProcessor] Matched "${student.studentName}" to DB student "${match.name}" (${match._id})`);
                return {
                    ...student,
                    studentId: match._id,
                ...
                console.log(`[AttendanceProcessor] No match found for "${student.studentName}"`);
                return student;
            }
        });
