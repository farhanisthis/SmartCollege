...
class AIProviderManager {
    ...
    initializeGeminiInstances() {
        ...
        geminiProviders.forEach((provider) => {
            try {
                ...
                console.error(`[AI Manager] Failed to initialize ${provider.name}:`, error);
                aiConfig.disableProvider(provider.name);
            }
        });
        console.log(`[AI Manager] Initialized ${this.geminiInstances.size} Gemini instances (memory optimized)`);
    }
    ...
    async useHuggingFace(prompt, model, providerName) {
        const hfProviders = aiConfig.getProvidersByType("huggingface");
        const availableProviders = hfProviders.filter((p) => !this.failedProviders.has(p.name));
        ...
        if (!targetProvider) {
            return {
                success: false,
                error: `HuggingFace provider ${providerName || "default"} not available`,
        ...
        try {
            console.log(`[AI Manager] Using HuggingFace ${targetProvider.name} with model ${selectedModel}`);
            ...
            console.error(`[AI Manager] Hugging Face error (${targetProvider.name}):`, error);
            this.markProviderAsFailed(targetProvider.name);
            // Try next provider if available
            const nextProvider = availableProviders.find((p) => p.name !== targetProvider.name);
    ...
    getHFModelParameters(model, prompt) {
    ...
    extractHFResponse(result, model) {
    ...
    async useGemini(model, prompt, providerName) {
        ...
        const availableProviders = geminiProviders.filter((p) => !this.failedProviders.has(p.name));
        if (availableProviders.length === 0) {
            return {
                success: false,
        ...
        try {
            ...
            console.error(`[AI Manager] Gemini error (${targetProvider.name}):`, error);
    ...
    async generateWithFallback(prompt, preferredType) {
        ...
        for (const provider of providers) {
            ...
            if (provider.type === "huggingface") {
                result = await this.useHuggingFace(prompt, provider.model, provider.name);
            }
            else {
                result = await this.useGemini(provider.model || "gemini-1.5-flash", prompt, provider.name);
            }
            if (result.success) {
    ...
    markProviderAsFailed(providerName) {
    ...
    getStatus() {
        ...
        const huggingFaceCount = allProviders.filter((p) => p.type === "huggingface").length;
        return {
    ...
    async testProvider(providerName) {
        ...
        if (provider.type === "huggingface") {
            return await this.useHuggingFace(testPrompt, provider.model, providerName);
        }
        else {
            return await this.useGemini(provider.model || "gemini-1.5-flash", testPrompt, providerName);
        }
    }
}
...
export const useHuggingFace = (prompt) => aiManager.useHuggingFace(prompt);
export const useGemini = (model, prompt) => aiManager.useGemini(model, prompt);
export const generateWithFallback = (prompt, preferredProvider) => aiManager.generateWithFallback(prompt, preferredProvider);
