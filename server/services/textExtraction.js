...
import { attendanceTextParser, } from "./attendanceTextParser";
export class TextExtractionService {
     ...
     */
    async extractFromAttendanceSheet(filePath) {
        console.log(`[OCR] Using attendance-optimized extraction (memory optimized) for: ${path.basename(filePath)}`);
        ...
        const strategies = [
            ...
            },
            {
                ...
                type: "tesseract",
                config: {
                    logger: (_m) => { }, // Suppress tesseract logs
        ...
        for (const strategy of strategies) {
            try {
                ...
                if (strategy.type === "ocrspace") {
                    // Use OCR Space API
                    const result = await ocrSpaceService.extractText(filePath, strategy.config);
                    if (result.success) {
                        text = result.content;
                        console.log(`[OCR] ${strategy.name}: ${text.length} chars, confidence: ${result.confidence}%`);
                    }
                    else {
                    ...
                    try {
                        ...
                        const { data: { text: tesseractText }, } = await worker.recognize(filePath);
                        text = tesseractText;
                    }
                    finally {
                ...
                console.log(`[OCR] ${strategy.name}: ${text.length} chars, score: ${score}`);
                if (score > bestScore ||
        ...
        console.log(`[OCR] Best result from ${bestStrategy} strategy: ${bestResult.length} characters, score: ${bestScore}`);
        ...
        try {
            ...
            attendanceData = await attendanceTextParser.parseAttendanceText(bestResult);
            console.log(`[AI] Successfully parsed attendance data: ${attendanceData.success ? attendanceData.data?.length || 0 : 0} students found`);
        }
        catch (error) {
     ...
     */
    async extractFromGeneralImage(filePath) {
        let worker;
        try {
            console.log(`[OCR] Using general image extraction for: ${path.basename(filePath)}`);
            worker = await createWorker("eng");
