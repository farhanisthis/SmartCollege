...
export class OcrSpaceService {
    ...
    async extractText(filePath, options = {}) {
        ...
        try {
            ...
            if (stats.size > 1024 * 1024) {
                console.warn(`[OCR Space] File size exceeds 1MB limit: ${stats.size} bytes`);
            }
     ...
     */
    async extractTextFromBase64(base64Image, options = {}) {
        const startTime = Date.now();
        try {
            ...
            formParams.append("detectOrientation", options.detectOrientation ? "true" : "false");
            ...
            console.log(`[OCR Space] API Response - Exit Code: ${result.OCRExitCode}`);
            return this.parseOcrSpaceResponse(result, processingTime, options.engine || 2);
        }
        catch (error) {
     ...
     */
    parseOcrSpaceResponse(response, processingTime, engineUsed) {
        try {
            ...
            const confidence = this.calculateConfidence(content, response.OCRExitCode);
            console.log(`[OCR Space] Successfully extracted ${content.length} characters`);
            return {
                success: true,
            ...
            return {
                ...
                processingTime,
                engineUsed,
                error: `Failed to parse OCR response: ${error instanceof Error ? error.message : String(error)}`,
            };
        }
    }
    /**
     ...
     */
    calculateConfidence(content, exitCode) {
        let confidence = 0;
        ...
        const studentNames = content.match(/\b[A-Z][a-z]+(\s+[A-Z][a-z]+)*\b/g) || [];
...
export const ocrSpaceService = new OcrSpaceService(process.env.OCR_SPACE_API_KEY || "");
