...
async function callGeminiWithRetry({ prompt, cacheKey, modelInstance, args = [], maxRetries = 3, }) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            const result = await limit(() => modelInstance.generateContent([prompt, ...args]));
            ...
            if (error.status === 429 && error.errorDetails) {
                ...
                for (const detail of error.errorDetails) {
                    if (detail["@type"]?.includes("RetryInfo") && detail.retryDelay) {
                        ...
                        if (match)
                            retryDelay = parseInt(match[1], 10) * 1000;
                    }
                }
...
function manuallyDetectDeadline(content, currentDate) {
    const result = {
    ...
    const hasDeadlineKeyword = deadlineKeywords.some((keyword) => contentLower.includes(keyword));
...
export async function formatContent(rawContent, detectedCategory) {
    let prompt;
...
${rawContent}`;
    ...
    const cacheKey = hashKey("format", rawContent, JSON.stringify(detectedCategory));
    // Check cache first
    if (aiCache.has(cacheKey)) {
        return aiCache.get(cacheKey);
    }
    try {
        console.log(`[formatContent] Starting formatContent for category: ${detectedCategory.category}`);
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (result.success && result.data) {
                    let text = result.data;
                    console.log(`[formatContent] AI Response for category "${detectedCategory.category}":`, text);
                    ...
                    if (text
                        ...
                        console.warn(`[formatContent] AI returned similar content to input, retrying...`);
                    ...
                    try {
                        ...
                        if (parsedResult.description.trim() === rawContent.trim() &&
                        ...
                        console.error(`[formatContent] Attempt ${attempt} JSON Parse Error:`, parseError);
                        console.error(`[formatContent] Raw text that failed to parse:`, text);
                ...
                console.error(`[formatContent] Attempt ${attempt} failed:`, attemptError);
        ...
        console.log(`[formatContent] All ${maxRetries} attempts failed, trying fallback`);
        // Fallback to original implementation if AI manager fails
        if (model) {
        ...
        const fallbackDescription = rawContent.length > 100
...
export async function analyzeImageContent(base64Image) {
    const prompt = "Analyze this image and extract any text content, especially if it contains academic information like assignments, notes, announcements, or presentations. Provide a detailed transcription of any text found.";
    ...
    try {
        ...
        const hfResult = await aiManager.useHuggingFace(`Image Analysis: ${prompt}`);
...
export async function processContentWithFiles(contextText, extractedTexts = []) {
