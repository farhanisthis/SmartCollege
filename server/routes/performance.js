...
import { AssignmentSubmissionModel, AttendanceModel, PresentationModel, UpdateModel, UserModel, } from "../models/mongodb";
import { nanoid } from "nanoid";
const router = Router();
// Helper to extract section from class string
function getSection(classStr) {
...
async function calculatePerformanceMetrics(userId, section, subject) {
    ...
    const presentationCompletion = allPresentations.length > 0
...
router.post("/debug/reset-sample-data", requireAuth, async (req, res) => {
...
router.get("/debug/assignments", requireAuth, async (req, res) => {
    try {
...
router.post("/debug/clear-submissions", requireAuth, async (req, res) => {
    try {
        const deletedCount = await AssignmentSubmissionModel.deleteMany({});
        console.log(`Cleared ${deletedCount.deletedCount} assignment submissions`);
        res.json({
            success: true,
...
router.get("/metrics", requireAuth, async (req, res) => {
    try {
        ...
        console.log(`[Performance Metrics] User ${userId}, RollNumber: ${rollNumber}, Section: ${section}`);
        const metrics = await calculatePerformanceMetrics(rollNumber, // Use rollNumber instead of userId
...
router.get("/dashboard", requireAuth, async (req, res) => {
    try {
        ...
        console.log(`[Performance Dashboard] User ${userId}, RollNumber: ${rollNumber}, Section: ${section}`);
        if (!section) {
        ...
        const [recentAttendanceRecords, allAssignments, allPresentationUpdates, userSubmissions, userPresentations, allUserPresentations,] = await Promise.all([
        ...
        recentAttendance.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        ...
        const validSubmissions = userSubmissions.filter((sub) => allAssignmentIds.includes(sub.updateId.toString()));
        const submittedAssignmentIds = validSubmissions.map((sub) => sub.updateId.toString());
        console.log(`Optimized: ${allAssignments.length} assignments, ${submittedAssignmentIds.length} submitted`);
        ...
        const allPresentationIds = allPresentationUpdates.map((p) => p._id.toString());
        // Filter user presentations for completed ones (data already fetched)
        const completedUserPresentations = allUserPresentations.filter((pres) => pres.status === "completed" &&
            allPresentationIds.includes(pres.updateId.toString()));
        const completedPresentationIds = completedUserPresentations.map((p) => p.updateId.toString());
        // Pending presentations (not completed) - optimized filtering
        const userPendingPresentations = allPresentationUpdates.filter((presentation) => !completedPresentationIds.includes(presentation._id.toString()));
        ...
        for (const presentation of userPresentations) {
            const update = allPresentationUpdates.find((u) => u._id.toString() === presentation.updateId.toString());
        ...
        const assignmentCompletion = totalAssignments > 0 ? (totalSubmissions / totalAssignments) * 100 : 0;
        ...
        const presentationCompletion = totalPresentations > 0
        ...
        const attendanceMetrics = await calculatePerformanceMetrics(rollNumber, // Use rollNumber instead of userId for attendance matching
        section, // Pass section
        subject);
        const metrics = {
            ...
            assignmentCompletion: Math.round(assignmentCompletion * 100) / 100,
            presentationCompletion: Math.round(presentationCompletion * 100) / 100,
            overallScore: Math.round((attendanceMetrics.attendancePercentage * 0.3 +
                assignmentCompletion * 0.4 +
                presentationCompletion * 0.3) *
                100) / 100,
        };
        ...
        const assignmentSubjects = Array.from(new Set(allAssignments.map((a) => a.subject).filter(Boolean)));
        const presentationSubjects = Array.from(new Set(allPresentationUpdates.map((p) => p.subject).filter(Boolean)));
        const allSubjects = Array.from(new Set([...assignmentSubjects, ...presentationSubjects]));
        const subjectPerformance = allSubjects
            ...
            const subjectPresentations = allPresentationUpdates.filter((p) => p.subject === subject);
            // Filter user submissions for this subject (from already fetched data)
            const subjectSubmissions = validSubmissions.filter((sub) => subjectAssignments.some((a) => a._id.toString() === sub.updateId.toString()));
            const subjectCompletedPresentations = completedUserPresentations.filter((pres) => subjectPresentations.some((p) => p._id.toString() === pres.updateId.toString()));
            const totalItems = subjectAssignments.length + subjectPresentations.length;
            const totalCompleted = subjectSubmissions.length + subjectCompletedPresentations.length;
            const overallScore = totalItems > 0 ? (totalCompleted / totalItems) * 100 : 0;
            return totalItems > 0
                ? {
                    subject,
        ...
        console.log(`✅ Optimized Dashboard: ${allAssignments.length} assignments, ${allPresentationUpdates.length} presentations, ${recentAttendance.length} recent attendance`);
...
// Submit assignment
router.post("/assignments/:updateId/submit", requireAuth, async (req, res) => {
    try {
...
router.get("/assignments/:updateId/submissions", requireAuth, async (req, res) => {
    try {
        const { updateId } = req.params;
        // Check if user is CR
        const user = await UserModel.findById(req.session.userId);
...
router.get("/attendance/class", requireAuth, async (req, res) => {
    try {
        const crId = req.session.userId;
...
router.post("/presentations", requireAuth, async (req, res) => {
    try {
...
router.put("/presentations/:id/score", requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
...
router.put("/presentations/:id/complete", requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.session.userId;
        const presentation = await PresentationModel.findById(id);
...
router.post("/presentations/:updateId/submit", requireAuth, async (req, res) => {
    try {
        const { updateId } = req.params;
        const userId = req.session.userId;
        console.log(`Presentation submission attempt: updateId=${updateId}, userId=${userId}`);
        ...
        if (presentationRecord) {
            ...
            console.log("Presentation record updated successfully with score:", score);
        }
        else {
...
router.post("/debug/fix-presentation-scores", async (req, res) => {
    try {
        ...
        console.log(`Found ${presentationsToFix.length} presentations with hardcoded score of 85`);
        let updatedCount = 0;
        for (const presentation of presentationsToFix) {
            ...
            await presentation.save();
            updatedCount++;
            console.log(`Updated presentation ${presentation._id} with new score: ${score}`);
        }
        res.json({
