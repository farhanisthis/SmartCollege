...
import { processAttendanceSheet, } from "../services/attendanceProcessor";
...
function getSubjectsForDay(dayOfWeek) {
    // E1 Timetable Data
...
const requireCRRole = (req, res, next) => {
    if (req.user?.role !== "cr") {
...
router.post("/save-day", requireAuth, requireCRRole, async (req, res) => {
    try {
        ...
        Object.entries(attendance).forEach(([studentId, statusOrSubjects]) => {
            ...
            if (typeof statusOrSubjects === "string") {
                ...
                Object.entries(statusOrSubjects).forEach(([subject, status]) => {
...
router.get("/history", requireAuth, async (req, res) => {
    try {
        ...
        const query = { classSection: section }; // Dynamic
...
router.get("/stats", requireAuth, async (req, res) => {
    try {
        ...
        const section = getSection(req.user?.class || "");
        if (!section)
            return res.json({ success: true, data: { totalDays: 0, avgAttendance: 0, subjectWiseStats: {}, dailyStats: [] } });
        ...
        console.log(`Calculating attendance stats for ${currentMonth}/${currentYear} Section: ${section}`);
        ...
        console.log(`Found ${records.length} records for stats calculation`);
        ...
        const stats = {
            ...
            avgAttendance: totalPossibleCount > 0
...
router.delete("/daily/:date", requireAuth, requireCRRole, async (req, res) => {
    try {
...
router.get("/timetable", requireAuth, async (req, res) => {
    try {
        ...
        const dayOfWeek = format(new Date(date), "EEEE"); // Monday, Tuesday, etc.
        // E1 Timetable Data
        const E1Schedule = {
            "10:30 AM—11:30 AM": {
                Monday: {
...
router.post("/upload-sheet-test", upload.single("attendanceSheet"), async (req, res) => {
    try {
        const file = req.file;
        ...
        let extractedText = "";
        try {
            const extractionResult = await textExtractionService.extractText(file.path);
            ...
            if (!extractedText || extractedText.trim().length < 20) {
                return res.status(400).json({
                    success: false,
                    error: "Could not extract sufficient text from the uploaded file. Please ensure the image is clear and contains readable text.",
                });
            }
        }
            ...
            return res.status(500).json({
                ...
                details: extractionError instanceof Error
        ...
        const processingResult = await processAttendanceSheet(extractedText, file.originalname, date || overrideDate);
...
router.post("/upload-text", requireAuth, requireCRRole, async (req, res) => {
    try {
        ...
        console.log(`[Text Upload] Text length: ${attendanceText.length} characters`);
        ...
        const processingResult = await processAttendanceSheet(attendanceText.trim(), "manual-text-input", overrideDate);
        if (!processingResult.success || !processingResult.data) {
            return res.status(400).json({
                success: false,
        ...
        const parsedData = processingResult.data;
        console.log(`[Text Upload] AI processed data: ${parsedData.students.length} students, ${parsedData.subjects.length} subjects`);
        ...
        parsedData.students.forEach((student) => {
            if (student.rollNumber) {
                ...
                console.log(`[Text Upload] Added student: ${student.rollNumber} (${student.studentName})`);
            }
            else {
                console.warn(`[Text Upload] Skipping student without enrollment number: ${student.studentName}`);
            }
        });
        ...
        const convertToFrontendSubjectName = (dbSubjectName) => {
        ...
        const students = Object.entries(attendanceData)
            ...
            const subjectsArray = Object.entries(subjectsData)
                .filter(([subject, _]) => validSubjects.some((vs) => vs.toLowerCase() === subject.toLowerCase() ||
        ...
        console.log(`[Text Upload] Attendance saved successfully: ${attendanceRecord._id}`);
...
router.post("/upload-sheet", requireAuth, requireCRRole, upload.single("attendanceSheet"), async (req, res) => {
    try {
        const file = req.file;
        const { date, overrideDate } = req.body;
        if (!file) {
        ...
        try {
            ...
            if (!extractedText || extractedText.trim().length < 20) {
                return res.status(400).json({
                    success: false,
                    error: "Could not extract sufficient text from the uploaded file. Please ensure the file contains readable attendance data.",
                    debug: {
            ...
            return res.status(500).json({
                success: false,
                error: "Failed to extract text from the uploaded file. Please try with a different file format.",
                debug: {
        ...
        console.log(`[Upload] Step 2: Processing attendance data with AI`);
        const processingResult = await processAttendanceSheet(extractedText, file.originalname, overrideDate || date);
        if (!processingResult.success || !processingResult.data) {
            return res.status(400).json({
                success: false,
        ...
        const parsedData = processingResult.data;
        console.log(`[Upload] AI processed data: ${parsedData.students.length} students, ${parsedData.subjects.length} subjects`);
        ...
        parsedData.students.forEach((student) => {
            if (student.rollNumber) {
                ...
                attendanceData[student.rollNumber] = student.subjects;
                console.log(`[Upload] Added student: ${student.rollNumber} (${student.studentName})`);
            }
            else {
                console.warn(`[Upload] Skipping student without enrollment number: ${student.studentName}`);
            }
        });
        ...
        console.log(`[Upload] Using date: ${attendanceDate} (override: ${overrideDate}, selected: ${date}, AI parsed: ${parsedData.date})`);
        const userId = req.user?.userId;
        // Use the existing save logic (similar to save-day endpoint)
        const validSubjects = [
            "Computer Graphics",
        ...
        console.log(`[Upload] Attendance saved successfully: ${attendanceRecord._id}`);
        const processedCount = parsedData.students.length;
        const savedCount = Object.keys(attendanceData).length;
        const skippedCount = processedCount - savedCount;
        // Return success response with processing details
...
router.post("/test-enhanced-ocr", upload.single("image"), async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
            return res.status(400).json({
        ...
        const enrollmentNumbers = extractedText.content.match(/\b\d{11}\b/g) || [];
        const studentNames = extractedText.content.match(/\b[A-Z][a-z]+(\s+[A-Z][a-z]+)*\b/g) || [];
...
router.post("/test-ocr-space", upload.single("image"), async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
            return res.status(400).json({
        ...
        console.log(`[OCR Space Test] Testing OCR Space on: ${file.originalname}`);
        ...
        // Analyze results
        const analyzeResult = (result, engineName) => {
            if (!result.success)
                return null;
            const enrollmentNumbers = result.content.match(/\b\d{11}\b/g) || [];
            const studentNames = result.content.match(/\b[A-Z][a-z]+(\s+[A-Z][a-z]+)*\b/g) || [];
...
router.post("/test-complete-pipeline", upload.single("image"), async (req, res) => {
    try {
        const file = req.file;
        if (!file) {
            return res.status(400).json({
        ...
        if (!hasAttendanceData && extractedText.content.trim().length > 0) {
            try {
                const { attendanceTextParser } = await import("../services/attendanceTextParser");
                manualParsingResult = await attendanceTextParser.parseAttendanceText(extractedText.content);
            }
            catch (error) {
...
router.post("/test-ai-parsing", async (req, res) => {
    try {
        const { attendanceTextParser } = await import("../services/attendanceTextParser");
        // Sample OCR text for testing
        const sampleOcrText = req.body.text ||
            `Enroll No Name [4 [9
...
01124402023 DEEPALI JAIN A A`;
        console.log(`[AI Parsing Test] Testing with ${sampleOcrText.length} characters of OCR text`);
        const result = await attendanceTextParser.parseAttendanceText(sampleOcrText);
        res.json({
            success: true,
            input: {
...
router.post("/test-ai-processing", async (req, res) => {
    try {
        const testText = `
    ...
    `;
        ...
        const processingResult = await processAttendanceSheet(testText, "test-attendance.txt");
        res.json({
            success: true,
            processingResult,
            testData: testText,
