...
import { UserModel, AttendanceModel, PerformanceMetricsModel, AssignmentSubmissionModel, PresentationModel } from "../models/mongodb";
const router = Router();
...
// Warning: devastating.
router.post("/nuke-db", requireAuth, async (req, res) => {
    try {
...
router.post("/create-e1-students", requireAuth, async (req, res) => {
    try {
        const password = "123123"; // Plain text password to match existing system
        const createdUsers = [];
        const skippedUsers = [];
        console.log(`Starting bulk user creation for ${E1Students.length} E1 students and ${E2Students.length} E2 students...`);
        const allStudents = [
        ...
        for (const student of allStudents) {
            try {
                ...
                if (existingUser) {
                    console.log(`User already exists: ${student.name} (${student.enrollment})`);
                    skippedUsers.push({
                        name: student.name,
                ...
                console.error(`Error creating user ${student.name}:`, userError.message);
                skippedUsers.push({
                    name: student.name,
                    enrollment: student.enrollment,
...
router.post("/fix-cr-data", requireAuth, async (req, res) => {
    try {
        const userId = req.session?.userId;
        if (!userId)
...
router.get("/e1-students", requireAuth, async (req, res) => {
    try {
        const userId = req.session?.userId;
        if (!userId)
...
router.delete("/e1-students", requireAuth, async (req, res) => {
    try {
...
router.delete("/reset-e1", requireAuth, async (req, res) => {
    try {
