...
const getSubjectsForDay = (schedule: any, dayName: string) => {
  ...
  Object.entries(schedule).forEach(
    ([timeSlot, scheduleData]: [string, any]) => {
...
export default function AttendanceManager() {
  ...
  const [customSchedule, setCustomSchedule] = useState<
    Record<string, Array<{ time: string; subject: string; bg: string }>>
  >(() => {
  ...
  const [editingSubjects, setEditingSubjects] = useState<
    Array<{ time: string; subject: string; bg: string }>
  >([]);
  ...
  const handleSubjectChange = (
    index: number,
    field: keyof (typeof editingSubjects)[0],
    value: string,
  ) => {
    const newSubjects = [...editingSubjects];
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const response = await fetch(getApiUrl("/api/bulk-users/e1-students"), {
          credentials: "include",
        });
    ...
    const loadExistingAttendance = async () => {
      ...
      try {
        const response = await fetch(
          getApiUrl(`/api/attendance/daily?date=${format(date, "yyyy-MM-dd")}`),
          {
            credentials: "include",
          },
        );
          ...
          if (result.success && result.data) {
            ...
            if (result.data.students) {
              ...
              result.data.students.forEach((studentRecord: any) => {
                ...
                const targetId =
                  enrollmentToIdMap.get(studentRecord.studentId) ||
                  studentRecord.studentId;

                if (loadedAttendance[targetId]) {
                  studentRecord.subjects.forEach((subjectRecord: any) => {
                    loadedAttendance[targetId][subjectRecord.subjectName] =
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl("/api/attendance/save-day"), {
        method: "POST",
        headers: {
  ...
  const handleExport = () => {
    ...
    const exportData = students.map((student) => {
      ...
      subjectsForDay.forEach((sub) => {
        row[sub.subject] =
          (studentAttendance[sub.subject] as string) || "Unmarked";
      });
        ...
        <div className="px-4 pt-4 space-y-4">
          <MobileAttendanceHeader
            ...
            onMarkAllPresent={() =>
              ...
              markAllPresent(currentActiveSubject.subject)
            }
            onMarkAllAbsent={() =>
              currentActiveSubject &&
              markAllAbsent(currentActiveSubject.subject)
            }
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center justify-between px-2">
                ...
                <Button
                  ...
                  className="text-[#ea580c] hover:text-[#c2410c] hover:bg-orange-50"
                >
              ...
              {subjectsForDay.map((subject, idx) => (
                <div
                  key={idx}
                  className="flex gap-4 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm items-center"
                >
                  <div
                    className={cn(
                      "h-16 w-16 rounded-xl bg-gradient-to-br flex items-center justify-center text-white font-bold text-xl shadow-lg",
                      subject.bg && subject.bg.includes("from-")
                        ? subject.bg
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="p-8 text-center bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                ...
                <p className="text-gray-500 text-sm">
                  Great job! You're all caught up for {dayName}.
                </p>
              </div>
            </div>
          )}
        ...
        <Dialog open={isEditSubjectsOpen} onOpenChange={setIsEditSubjectsOpen}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto w-[90vw] rounded-xl z-[60]">
            <DialogHeader>
              ...
              <DialogDescription>{format(date, "PPP")}</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="flex-col gap-2 sm:flex-col">
              ...
              <Button className="w-full" onClick={handleSaveSubjects}>
                Save Changes
              </Button>
              <Button
                variant="outline"
      ...
      <Dialog open={isEditSubjectsOpen} onOpenChange={setIsEditSubjectsOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Customize the subjects and time slots for this specific date.
              Changes apply only to this day.
            </DialogDescription>
          </DialogHeader>
